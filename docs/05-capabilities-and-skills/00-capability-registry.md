...
- Every capability has a `capability_id` only
- Agent requests capability by ID → always gets "latest"
- No semver, no version ranges, no compatibility checks

**Why start simple:**
- Faster iteration in early development
- Fewer edge cases to handle
- Versioning complexity added when actually needed

**Breaking changes:**
- Create new `capability_id` (e.g., "weights_v2")
- Migrate users explicitly
- Deprecate old capability over time

**Phase 2+ (future):**
- Add semantic versioning when needed
- Agent can request version ranges
- API Executor enforces version compatibility
